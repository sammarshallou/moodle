...
class user_fields {
    ...
    /**
     * Construct a user fields object, specifying which purposes you require the fields for.
     *
     ...
     * to get all the fields without checking permissions. It is also OK to pass context null
     * anyway if you are only getting the name or userpic fields, as these are always allowed.
     *
     * As well as the standard fields required for normal purposes, if you need specific fields in
     * your code, you can include extra fields here. You can also exclude fields if they do not
     * need to be included.
     *
     * The list of included and excluded fields can include fields directly in the user table
     * e.g. 'country', and also user profile fields e.g. 'profile_field_myfield'.
     *
     * @param \context|null Context; if not supplied, all fields will be included without checks
     * @param int[] $purposes Array of required PURPOSE_xx constants
     * @param bool $allowcustom If true, custom profile fields may be included
     * @param string[] $include Array of extra fields to include
     * @param string[] $exclude Array of fields to exclude
     * @throws \coding_exception If any unknown purpose is listed
     */
    public function __construct(?\context $context, array $purposes, bool $allowcustom = true,
            array $include = [], array $exclude = []) {
        $this->context = $context;
        $this->purposes = self::check_and_convert_purposes($purposes);
    ...
    /**
     * Converts purposes from an input array to associative array of flags.
     *
     * @param array $purposes Input array of purposes (array of PURPOSE_xx constants)
     * @return bool[] Array of purposes from purpose constant => true or false
     * @throws \coding_exception If any unknown purpose is listed
     */
    protected static function check_and_convert_purposes(array $purposes): array {
        $result = [
        ...
        foreach ($purposes as $purpose) {
            if (array_key_exists($purpose, $result)) {
                ...
                throw new \coding_exception('Unknown purpose constant ' . $purpose);
            }
        }
        return $result;
    }
        ...
        if ($limitpurposes) {
            ...
            foreach ($limitpurposes as $purpose) {
                if ($purpose != self::CUSTOM_INCLUDE && empty($this->purposes[$purpose])) {
                    throw new \coding_exception('$limitpurposes can only include purposes specified in constructor');
                }
            }
     ...
     *
     * The result of this function is an associative array with keys 'selects', 'joins',
     * 'params', 'mappings'.
     *
     ...
     * @return array Associative array with necessary SQL components
     */
    public function get_sql(string $alias = '', bool $namedparams = false, string $prefix = '',
            string $renameid = '', bool $leadingcomma = true): array {
        global $DB;
        ...
        return ['selects' => $selects, 'joins' => $joins, 'params' => $params,
                'mappings' => $mappings];
    }
