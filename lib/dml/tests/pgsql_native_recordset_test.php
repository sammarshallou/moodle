...
class pgsql_native_recordset_testcase extends basic_testcase {
     ...
     * @param bool $stricttransactions True if strict transactions in use
     */
    protected function init_db($fetchbuffersize, $stricttransactions = false) {
        global $CFG, $DB;
        ...
        $dboptions = ['fetchbuffersize' => $fetchbuffersize,
                'stricttransactions' => $stricttransactions];
    ...
    /**
     * Tests that the database throws an exception if you commit all transactions before closing
     * recordsets, when strict transactions are in use.
     */
    public function test_recordset_strict_transactions() {
        $this->init_db(3, true);
        ...
        try {
            ...
            $this->assertContains('before closing recordsets', $e->getMessage());
        }
    }
