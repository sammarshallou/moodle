...
class user_fields_testcase extends \advanced_testcase {
    ...
    public function test_get_required_fields() {
        ...
        $fields = new user_fields(null, [user_fields::PURPOSE_IDENTITY, user_fields::PURPOSE_NAME,
                user_fields::PURPOSE_USERPIC]);
        ...
        $fields = new user_fields(null, [user_fields::PURPOSE_IDENTITY], true, ['city', 'profile_field_b']);
        ...
        $fields = new user_fields(null, [user_fields::PURPOSE_IDENTITY], true, [], ['email', 'city']);
        ...
        $fields = new user_fields(null, [user_fields::PURPOSE_IDENTITY], false, ['city', 'profile_field_b'],
                ['city', 'department']);
    ...
    public function test_get_required_fields_limitpurposes() {
        ...
        $fields = new user_fields(null, [user_fields::PURPOSE_IDENTITY, user_fields::PURPOSE_NAME,
                user_fields::PURPOSE_USERPIC], true, ['city', 'profile_field_b'],
    ...
    public function test_get_required_fields_limitpurposes_not_in_constructor() {
        $fields = new user_fields(null, [user_fields::PURPOSE_IDENTITY]);
        $this->expectExceptionMessage('$limitpurposes can only include purposes specified in constructor');
    ...
    protected function init_for_sql_tests(): user_fields {
        ...
        return new user_fields(null, [], true, ['department', 'city', 'profile_field_a', 'profile_field_b']);
    }
    ...
    public function test_get_sql_variations() {
        ...
        ['selects' => $selects, 'joins' => $joins, 'params' => $joinparams] =
                $fields->get_sql('', false, 'u_', 'userid');
        $sql = "SELECT idnumber 
                       $selects 
                  FROM {user}
                       $joins
    ...
    public function test_get_sql_multiple() {
        ...
        ['selects' => $selects1, 'joins' => $joins1, 'params' => $joinparams1] =
                $fields->get_sql('u1', true);
        // Outer SQL.
        $fields2 = new user_fields(null, [], true, ['profile_field_a', 'email']);
        ['selects' => $selects2, 'joins' => $joins2, 'params' => $joinparams2] =
                $fields2->get_sql('u2', true);
        ...
        $expected1 = (object)['username' => 'u1', 'innerb' => 'B1', 'innerc' => 'C1', 'profile_field_a' => 'A1', 'email' => 'e1@example.org'];
    ...
    public function test_get_sql_nothing() {
        $fields = new user_fields(null, []);
        ['selects' => $selects, 'joins' => $joins, 'params' => $joinparams] = $fields->get_sql();
    ...
    public function test_get_sql_no_custom_fields() {
        $fields = new user_fields(null, [], true, ['city', 'country']);
        ['selects' => $selects, 'joins' => $joins, 'params' => $joinparams, 'mappings' => $mappings] =
                $fields->get_sql('u');
    ...
    public function test_get_sql_selects_format() {
        ...
        $fields = new user_fields(null, [], true, ['id', 'profile_field_a']);

        // Supplying an alias: all fields have alias.
        ['selects' => $selects] = $fields->get_sql('u');
        $this->assertEquals(', u.id, uf1d_1.data AS profile_field_a', $selects);

        // No alias: all files have {user} because of the joins.
        ['selects' => $selects] = $fields->get_sql();
        $this->assertEquals(', {user}.id, uf2d_1.data AS profile_field_a', $selects);

        // When the list doesn't include custom profile fields...
        $fields = new user_fields(null, [], true, ['id', 'city']);
