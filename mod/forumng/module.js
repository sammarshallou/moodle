M.mod_forumng = {
    ...
    init_reply: function(link, replytoid) {
        link.on('click', function(e) {
            ...
            form.remove();
            form.post.insertBefore(form, form.post.get('lastChild'));
    ...
    init_editor_and_attachments: function(form, settings) {
        form.expectingeditor = form.get('tryinghtmleditor').get('value') == '1';
        form.usingeditor = form.expectingeditor && window.tinyMCE;
    ...
    init_expand: function(link, postid, expandnow) {
        ...
        link.loader = link.get('nextSibling').get('nextSibling');
        ...
        link.set('innerHTML', M.str.forumng.expand.replace('#', postnum));
    ...
    expand_ok: function(transactionid, o, link) {
        ...
        if (this.select.on) {
            previousSelect = this.Y.one('#id_selectp' + link.postid).get('checked');
        }
        ...
        this.init_content(newPost);
        if (previousSelect) {
            var checkbox = this.Y.one('#id_selectp' + link.postid);
    ...
    select_init : function(target) {
        ...
        for (var i=0; i<posts.size(); i++) {
            this.select_init_post(posts.item(i), this.select.on, selectChanged);
        }
        if (this.select.on) {
            selectChanged();
        }
     ...
     * @param selectChanged Function to run when selection changes
     */
    select_init_post : function(post, on, selectChanged) {
        if (on) {
            ...
            check.on('click', function() {
                if (check.get('checked')) {
                    ...
                    post.hidden.set('value', 1);
                } else {
                    post.addClass('forumng-deselected');
                    post.hidden.set('value', 0);
                }
                selectChanged();
            }, this);
        } else {
    ...
    star_init_events : function(div, star) {
        ...
        star.clickfunction = function(e) {
            ...
            star.src = M.mod_forumng.loaderpix;
        };
