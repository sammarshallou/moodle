...
class mod_forumng_renderer extends plugin_renderer_base {
    ...
    public function render_discussion_list_start($forum, $groupid, $baseurl,
        ...
        foreach (array(mod_forumng::SORT_DATE, mod_forumng::SORT_SUBJECT, mod_forumng::SORT_AUTHOR,
            mod_forumng::SORT_POSTS, mod_forumng::SORT_UNREAD, mod_forumng::SORT_GROUP) as $possiblesort) {
            $data = new stdClass;
            if ($sort == $possiblesort) {
                $data->before = '<a ' . 'id="sortlink_' . mod_forumng::get_sort_letter($possiblesort) .
                    '" href="' . s($baseurl) . '&amp;sort=' .
                ...
                $data->before = '<a ' . 'id="sortlink_' . mod_forumng::get_sort_letter($possiblesort) .
                    '" href="' . s($baseurl) . '&amp;sort=' .
                    mod_forumng::get_sort_letter($possiblesort) . '&amp;sortlink=' .
                    mod_forumng::get_sort_letter($possiblesort) . '" class="forumng-sortlink" '.
                    'title="'. mod_forumng::get_sort_title($possiblesort) . ' ' .
     ...
     * @return string HTML code for this area
     */
    function render_discussion_subscribe_option($discussion, $subscribe) {
        global $USER;
        if ($subscribe) {
        ...
        if ($html) {
            ...
            if ($edituser = $post->get_edit_user()) {
                ...
                if ($options[mod_forumng_post::OPTION_COMMAND_HISTORY]) {
                    $out .= ' (<a href="history.php?' . $post->get_link_params(mod_forumng::PARAM_HTML) .
        ...
        if ($subject !== null && $expanded && !$deletedhide) {
            if ($html) {
                ...
                if ($options[mod_forumng_post::OPTION_DIGEST]) {
                    ...
                    $out .= " <{$linkprefix}discuss.php?" . $discussion->get_link_params(mod_forumng::PARAM_HTML) .
        ...
        if ($expanded && !$deletedhide) {
            ...
            if ($html) {
                ...
                if ($options[mod_forumng_post::OPTION_JUMP_PREVIOUS] ||
                    ...
                    if ($nextid = $options[mod_forumng_post::OPTION_JUMP_NEXT]) {
                        $commands .= ' <a href="#p'. $nextid . '" class="forumng-next">' .
                            get_string('jumpnext', 'forumng') . '</a>';
                    }
                    if ($pid = $options[mod_forumng_post::OPTION_JUMP_PREVIOUS]) {
                        if ($nextid) {
                    ...
                    if ($parentid = $options[mod_forumng_post::OPTION_JUMP_PARENT]) {
                ...
                if ($options[mod_forumng_post::OPTION_COMMAND_DIRECTLINK]) {
                    $commands .= '<li class="forumng-permalink"><a href="discuss.php?' . $discussion->get_link_params(mod_forumng::PARAM_HTML) . '#p' . $post->get_id() .
