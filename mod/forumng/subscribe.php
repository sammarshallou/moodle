...
function get_group_list($userid, $forumngid) {
    global $DB;
    //$courseid = mod_forumng::get_from_id($forumng->id, mod_forumng::CLONE_DIRECT)->get_course_id();
    $sql_group = "
SELECT
    g.id AS groupid
FROM
...
WHERE
    ...
    $rs = $DB->get_recordset_sql($sql_group, array($userid, $forumngid));
    $results = array();
    foreach ($rs as $rec) {
        $results[] = $rec->groupid;
    }
...
if ($cmid) {
    ...
    if ($groupid) {
        ...
        if ($forum->get_group_mode() == SEPARATEGROUPS && !$aaguser) {
            $group_list = get_group_list($userid, $forumngid);
