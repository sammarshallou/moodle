...
class mod_forumng_discussion {
    ...
    public static function get_from_id($id, $cloneid, $userid=0, $usecache=false, $storecache=false) {
        if ($usecache) {
            ...
            foreach ($SESSION->forumng_cache->discussions as $info) {
                if ($info->userid==mod_forumng_utils::get_real_userid($userid) && $info->id==$id && $info->cloneid==$cloneid) {
                    $info->lastused = time();
    ...
    function __construct($forum, $discussionfields, $full, $foruserid) {
        if ($full && !isset($discussionfields->firstuser)) {
            ...
            $discussionfields->firstuser = mod_forumng_utils::extract_subobject($discussionfields, 'fu_');
            $discussionfields->lastuser = mod_forumng_utils::extract_subobject($discussionfields, 'lu_');
        }
    ...
    fd.id = ?)";
        ...
        if (is_dir($folder)) {
            if (!remove_dir($folder)) {
                throw new mod_mod_forumng_file_exception("Error deleting attachment folder: $folder");
            }
        }
        //Log delete
        if ($log) {
...
WHERE
    ...
    AND fr.discussionid = ?
ORDER BY
    fr.time DESC", array_merge(array($id), $params));
