...
if ($mform->is_cancelled()) {
    if ($edit) {
        redirect('discuss.php?' . $post->get_discussion()->get_link_params(mod_forumng::PARAM_PLAIN));
    } else if ($islock) {
    ...
    if ($savedraft) {
        ...
        if ($draft) {
            ...
            $fromform->message['text'] = file_save_draft_area_files($fromform->message['itemid'],
                    $filecontext->id, 'mod_forumng', 'draftmessage', $draft->get_id(), $fileoptions,
        ...
        if ($isdiscussion) {
            ...
            list($discussionid, $postid) =
                $forum->create_discussion($groupid,
                        $fromform->subject, $fromform->message['text'], $fromform->message['format'],
                        $hasattachments, !empty($fromform->mailnow), $fromform->timestart,
            ...
            $postid = $discussion->lock($fromform->subject, $fromform->message['text'],
                    $fromform->message['format'], $attachments, !empty($fromform->mailnow));
            ...
            finish($postid, $cloneid,
                'discuss.php?' . $replyto->get_discussion()->get_link_params(mod_forumng::PARAM_PLAIN),
                $fromform);
        } else {
            // Create a new reply
            $transaction = $DB->start_delegated_transaction();
