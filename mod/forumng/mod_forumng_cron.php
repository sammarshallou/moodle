...
WHERE
    ...
    OR (fd.deleted<>0 AND fd.deleted<?)";
        ...
        if ($count) {
            ...
            $attachmentrecords = $DB->get_records_sql("
SELECT
    fp.id AS postid, fd.id AS discussionid, f.id AS forumngid, f.course AS courseid
$mainquery
    AND fp.attachments<>0");
            ...
            foreach ($attachmentrecords as $attachmentrecord) {
                $folder = mod_forumng_post::get_any_attachment_folder(
                        $attachmentrecord->courseid, $attachmentrecord->forumngid,
            ...
            foreach ($discussions as $attachmentrecord) {
                $folder = mod_forumng_discussion::get_attachment_folder(
                    $attachmentrecord->courseid, $attachmentrecord->forumngid,
    ...
    private static function digest_add_post_for_user(&$user, &$userdigests,
        ...
        if ($userdigests[$user->id]->discussionid != $discussion->get_id()) {
            ...
            $html .= "<div class='forumng-breadcrumbs'>" .
                ...
                "<a target='_blank' href='$CFG->wwwroot/mod/forumng/view.php?" . $forum->get_link_params(mod_forumng::PARAM_HTML) . "'>".format_string($forum->get_name(),true)."</a>";
            if ($discussion->get_subject(false) !== $forum->get_name()) {
