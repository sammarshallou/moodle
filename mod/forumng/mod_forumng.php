...
SELECT
    x.*, c.id AS courseid, c.shortname AS courseshortname, f.name AS forumname, f.id AS cloneforumngid, cm.section AS sectionid
FROM
    {forumng} f
    ...
    public function subscribe($userid=0, $groupid=null, $log=true) {
        ...
        if (!$groupid) {
            ...
            $DB->execute(
                    ...
                    "WHERE userid = ? AND forumngid = ? AND clonecmid $clonevalue AND subscribed = 1 " .
    ...
    public function is_in_auto_subscribe_list($userid=0, $expectingquery=false) {
        ...
        if ($groups) {
            if (isset($USER) && $userid == $USER->id) {
                ...
                list($inorequals, $inparams) = mod_forumng_utils::get_in_array_sql('groupid', $groups);
...
FROM
    ...
    LEFT JOIN {groups_members} discussiongm ON fd.groupid = discussiongm.groupid AND s.userid = discussiongm.userid
    LEFT JOIN {groups_members} subscriptiongm ON s.groupid = subscriptiongm.groupid AND s.userid = subscriptiongm.userid
WHERE
    s.forumngid = ?
    AND s.userid = ?
    ...
    private function grade_item_update($grades = array(), $idnumber=null) {
        ...
        if (is_null($idnumber)) {
            ...
            if (!property_exists($cm, 'idnumber')) {
                $cm->idnumber = $DB->get_field('course_modules', 'idnumber', array('id' => $cm->id));
            }
...
WHERE
    ...
    private static function query_forums($cmids=array(), $course=null,
        ...
        list($cfdinviewhiddenforums, $inviewhiddenforumsparams) = mod_forumng_utils::get_in_array_sql(
            'cfd.forumngid', $viewhiddenforums);
...
WHERE
    ...
    public function display_search_form($querytext='') {
        ...
        $linkfields = $this->get_link_params(mod_forumng::PARAM_FORM);
        $buttontext = !mod_forumng::search_installed() ? '' : <<<EOF
...
EOF;
    ...
    public function print_js($cmid=0, $ajaxattachments=false) {
        ...
        if (ajaxenabled()) {
            ...
            if ($ajaxattachments) {
                ...
                // To make it a bit shorter, let's remove the noscript part as it will never be used.
    ...
    public function get_flagged_posts($userid=0) {
        ...
        $records = mod_forumng_post::query_posts(
                ...
                array($this->get_id()), 'ff.flagged DESC', false, true, false, $userid, true, true);
    ...
    fp.userid = ? AND fd.forumngid = ? AND fp.deleted = 0 AND fd.deleted = 0";
        ...
        if ($this->forumfields->completiondiscussions) {
            $value = $this->forumfields->completiondiscussions <=
              $DB->count_records('mod_forumng_discussions',
                      array('forum' => $forumngid, 'userid' => $userid, 'deleted' => 0));
        ...
        if (!$nodata) {
            ...
            switch ($newforum->get_effective_subscription_option()) {
                case self::SUBSCRIPTION_PERMITTED:
                    ...
                    $rs = $DB->get_recordset('mod_forumng_subscriptions', array('forum' => $forum->id));
                    foreach ($rs as $rec) {
            ...
            foreach ($rsd as $recd) {
                ...
                foreach ($rsp as $recp) {
                    ...
                    if ($forumng->ratingscale) {
                        $rsr = $DB->get_recordset('mod_forumng_ratings', array('post' => $recp->id));
                        foreach ($rsr as $recr) {
...
WHERE
     ...
     * @return array Array of mod_forumng_post objects
     */
    public static function get_all_posts_by_user($userid, $groupid, $order = 'fp.id') {
...
ORDER BY
    subs.cmid, subs.subid";
        $rs = $DB->get_recordset_sql($sql_group, $optionalqueryparams);
        $results = array();
        foreach ($rs as $rec) {
        ...
        $rs->close();
        $rs = $DB->get_recordset_sql($sql_sub, $optionalqueryparams);
        $lastcmid = 0;
        $forumcount = 1;
        $totalforumcount = 0;
        $totalforumcount = $DB->count_records_sql($sql_count, $optionalqueryparams);
