...
class restore_forumng_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        if ($userinfo) {
            $paths[] = new restore_path_element('forumng_discussion', '/activity/forumng/discussions/discussion');
            $paths[] = new restore_path_element('forumng_post', '/activity/forumng/discussions/discussion/posts/post');
            $paths[] = new restore_path_element('forumng_rating', '/activity/forumng/discussions/discussion/posts/post/ratings/rating');
            $paths[] = new restore_path_element('forumng_flag', '/activity/forumng/discussions/discussion/posts/post/flags/flag');
            $paths[] = new restore_path_element('forumng_subscription', '/activity/forumng/subscriptions/subscription');
            $paths[] = new restore_path_element('forumng_read', '/activity/forumng/discussions/discussion/readdiscussions/read');
            $paths[] = new restore_path_element('forumng_draft', '/activity/forumng/drafts/draft');
        }
    ...
    protected function process_forumng_draft($data) {
        ...
        // If post has parent, map it (it has been already restored)
        if (!empty($data->parentpostid)) {
            $data->parentpostid = $this->get_mappingid_or_null('forumng_post', $data->parentpostid);
        }
    ...
    protected function process_forumng_subscription($data) {
        ...
        $data->forumngid = $this->get_new_parentid('forumng');
        $data->userid = $this->get_mappingid_or_null('user', $data->userid);
        $data->discussionid = $this->get_mappingid_or_null('forumng_discussion', $data->discussionid);
    ...
    protected function after_execute() {
        ...
        // Add forumng related files, no need to match by itemname (just internally handled context)
