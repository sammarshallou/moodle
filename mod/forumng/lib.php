...
function forumng_delete_instance($id) {
    ...
    $forum = mod_forumng::get_from_id($id, mod_forumng::CLONE_DIRECT);
    ...
    if ($forum->is_shared()) {
        ...
        foreach ($contexts as $context) {
            ...
            if (!delete_course_module($context->instanceid)) {
                notify("Could not delete the Clone forumng (coursemoduleid) $context->instanceid ");
                return false;
            }
...
function forumng_get_coursemodule_info($coursemodule) {
    global $DB;
    if ($forumng = $DB->get_record('forumng', array('id' => $coursemodule->instance), 'id, name, type')) {
        $info = new object();
...
function forumng_print_overview($courses,&$htmlarray) {
    ...
    foreach ($courses as $course) {
        $str = "";
        $forums = mod_forumng::get_course_forums($course, $USER->id, mod_forumng::UNREAD_DISCUSSIONS);
        if (!empty($forums)) {
...
function mod_forumng_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload) {
    ...
    $fs = get_file_storage();
    if (!($file = $fs->get_file($context->id, 'mod_forumng', $filearea, $itemid, '/', $filename)) ||
            $file->is_directory()) {
        send_file_not_found();
    }
