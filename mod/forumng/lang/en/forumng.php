...
$string['subscription_help'] = 'You can force everyone to be subscribed, or set them be subscribed initially; the difference is that in the latter case, they can choose to unsubscribe themselves.

These options include all users (students and staff) who are enrolled on the course. Users who do not belong to the course (such as administrators) can still optionally subscribe.';
...
$string['forumtype_help'] = 'Different types of forum are available for specific purposes or teaching methods. The standard forum type is appropriate for all normal use.';
...
$string['grading_help'] = 'If you select this option, a grade for this forum will be added to the course gradebook and calcualted automatically. Leave this off for a non-assessed forum, or one you plan to assess manually.

The different ways to calculate grading are fairly self-explanatory; in each case, the grade for each student is calculated based on all ratings for all posts they have made. Grades are limited to the scale; for example if the scale is 0-3, the grading method is set to &lsquo;count&rsquo; and the student&rsquo;s posts have received 17 ratings, their grade will be 3.';
...
$string['attachmentmaxbytes_help'] = 'This is the maximum <i>total</i> size for all attachments on a single post.';
$string['configattachmentmaxbytes'] = 'Default maximum size for all forum attachments on the site (subject to course limits and other local settings)';
$string['readafterdays'] = 'Read after days';
$string['configreadafterdays'] = 'After this number of days, posts are considered to have been read by all users.';
...
$string['enableratings_help'] = 'If enabled, forum posts can be given ratings using a numeric or defined Moodle scale. One or more people can rate the post and the displayed rating is the average (mean) of those ratings.

If you use a numeric scale up to 5 (or fewer) then a nice &lsquo;star&rsquo; display is used. Otherwise it&rsquo;s a dropdown.

The capabilities system controls who can rate posts and see ratings. By default, only teachers can rate posts, and students can only see ratings on their own posts.';
...
$string['feedtype_help']='If enabled, users can subscribe to the forum using an Atom or RSS feed reader. You can set the feed to include only top-level discussions and not replies, or to include all posts.';
...
$string['feeditems_help'] = 'The number of items included in the Atom/RSS feeds. If this is set low, then users who don&rsquo;t check the feed frequently might miss some messages.';
...
$string['enablelimit_help'] = 'This option limits discussions and replies made by students (specifically, any users who do not have the <tt>mod/forumng:ignorethrottling</tt> capability).

When a student is only permitted 3 more posts, a warning displays in the post form. After their limit runs out, the system displays the time at which they&rsquo;ll be able to post again.';
$string['completiondiscussions'] = 'User must create discussions:';
$string['completiondiscussionsgroup'] = 'Require discussions';
$string['completiondiscussionsgroup_help'] = 'If ticked, the forum will be marked complete for a student once they have started the required number of new discussions (and met any other conditions).';
$string['completionposts'] = 'User must post discussions or replies:';
$string['completionpostsgroup'] = 'Require posts';
$string['completionpostsgroup_help'] = 'If ticked, the forum will be marked complete for a student once they have created the required number of discussions/replies, counting each discussion or reply as one (and met any other conditions).';
$string['completionreplies'] = 'User must post replies:';
$string['completionrepliesgroup'] = 'Require replies';
$string['completionrepliesgroup_help'] = 'If ticked, the forum will be marked complete for a student once they have made the required number of replies to existing discussions (and met any other conditions).';
...
$string['ratingthreshold_help'] = 'If you set this option to 3, then the rating for a post will not be shown until at least 3 people have rated it.
...
$string['shared_help'] = 'Tick this box, and set the ID number field below, to enable this forum to be shared.

This forum will become the original forum. You can then create one or more copies of this forum by choosing <strong>Use existing shared forum</strong>, and typing in the same ID number, when creating each copy.';
$string['sharing'] = 'Forum sharing';
$string['useshared'] = 'Clone existing forum';
$string['useshared_help'] = 'If you want to create a clone of an existing forum, use this tickbox and type the ID number of the original forum (which must allow sharing).

When this is selected, most other options on this form will be ignored because you are not really creating a new forum, only a link to an existing one. The exception is availability and (manual only) completion options.';
...
// New error strings
$string['error_fileexception'] = 'A file processing error occurred. This is likely to be caused by system problems. You may wish to try again later.';
...
$string['reportingemail_help'] = 'If this email address is supplied, then a Report link appears next to each post. Users can click the link to report offensive posts. The information will be sent to this address.

If this email is left blank then the Report feature will not be shown (unless a site-level reporting  address has been supplied).';
...
$string['removeolddiscussions_help'] = 'The system can automatically remove discussions if they have not had any new replies for a certain length of time.';
...
<ul><li>Delete them permanently; unlike the standard delete feature, these cannot be undeleted. This option could be used to save space in the database.</li>
<li>Move them to another forum; for example, you might have an &lsquo;archive forum&rsquo;. You can select any forum on the same course.</li></ul>';
...
For example, you could match posts which contain the word frog, do not contain the word jump, and contain the phrase &lsquo;ribbit ribbit&rsquo; by typing <tt>frog -jump &quot;ribbit ribbit&quot;</tt> into this box.

If you leave this box blank, then all posts which match the author and/or date criteria will be returned regardless of their content.';
...
If you don&rsquo;t specify an author, then posts from all authors will be included in the results.';
$string['daterangefrom'] = 'Date range from';
$string['daterangefrom_help'] = 'Use the dates to restrict your search to only include posts within the given time range.
...
$string['error_makebig'] = 'The course only has {$a->users} users, but you\'ve asked for {$a->readusers} to read each discussion. Create some more users.';
$string['error_system'] = 'A system error occurred: {$a}';



$string['modulename_help'] = 'ForumNG is a replacement for standard Moodle forum with most of the same features plus additional ones and a more dynamic user interface.

NG stands for \'Next Generation\'.';

$string['mailnow_help'] = 'Send your post to email subscribers more quickly.

If you choose this option, the system doesn\'t wait to give you a chance to edit the post. Instead it is included in the next email processing batch.

You can still edit the post, but subscribers may have already received the original version.';
$string['displayperiod_help'] = 'You can hide this discussion from students until, or after, a certain date.

While hidden, students do not see the discussion at all. For moderators, it shows on the discussion list in grey and with a clock icon.';

$string['sticky_help'] = 'This option can make the discussion stay on top of the list, even after newer discussions are posted.

Sticky discussions are displayed with an up-arrow icon on the discussion list. You can have more than one sticky discussion.';

$string['errorfindinglastpost'] = 'Error recalculating last post (database inconsistent?)';

$string['drafts_help'] = 'When you save a post as draft, it appears on this list. Click on the draft to resume working on it.

If you want to delete the draft, click the delete icon next to it. There is a confirmation screen.

In some cases it may not be possible to continue your draft (for example if it is in reply to a discussion which has since been deleted). In this situation, you can retrieve the contents of your draft by clicking the delete icon.';

$string['flaggedposts_help'] = 'Flagged posts appear in this list. To jump to a flagged post, click on it.
