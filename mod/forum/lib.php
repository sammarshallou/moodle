...
function forum_print_recent_activity($course, $viewfullnames, $timestart) {
    ...
    ['selects' => $allnamefields] = $userfieldsapi->get_sql(null, false, false, 'u', '', 'duserid', false);
...
function forum_get_discussions($cm, $forumsort="", $fullpost=true, $unused=-1, $limit=-1,
    ...
    if (empty($userlastmodified)) {  // We don't need to know this
        ...
        $umfields = ', ' . $userfieldsapi->get_sql('um', false, 'um') . ', um.email AS umemail, um.picture AS umpicture,
...
function forum_get_posts_by_user($user, array $courses, $musthaveaccess = false, $discussionsonly = false, $limitfrom = 0, $limitnum = 50) {
    ...
    // oracle and mssql.
    $userfieldsapi = new \core\user_fields([\core\user_fields::PURPOSE_USERPIC]);
    ['selects' => $userfields] = $userfieldsapi->get_sql(null, false, false, 'u', '', 'useridx', false);
    $countsql = 'SELECT COUNT(*) ';
