...
function report_log_print_mnet_selector_form($hostid, $course, $selecteduser=0, $selecteddate='today',
    ...
    if ($hostid == $CFG->mnet_localhost_id && $course->id != SITEID) {
        ...
        $courseusers = get_enrolled_users($context, '', $selectedgroup, 'u.id, ' . $userfieldsapi->get_sql('u', false, '', '', false),
                null, $limitfrom, $limitnum);
    } else {
        // this may be a lot of users :-(
        $userfieldsapi = new \core\user_fields(null, [\core\user_fields::PURPOSE_NAME]);
        $courseusers = $DB->get_records('user', array('deleted'=>0), 'lastaccess DESC', 'id, ' . $userfieldsapi->get_sql('', false, '', '', false),
                $limitfrom, $limitnum);
    }
