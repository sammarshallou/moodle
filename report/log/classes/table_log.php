...
class report_log_table_log extends table_sql {
    ...
    protected function get_user_fullname($userid) {
        ...
        $sql = "SELECT id," . $userfieldsapi->get_sql('', false, '', '', false) . " FROM {user} WHERE id " . $usql;
    ...
    protected function update_users_used() {
        ...
        if (!empty($userids)) {
            ...
            $userfieldsapi = new \core\user_fields(null, [\core\user_fields::PURPOSE_NAME]);
            $users = $DB->get_records_sql("SELECT id," . $userfieldsapi->get_sql('', false, '', '', false) . " FROM {user} WHERE id " . $usql,
                    $uparams);
