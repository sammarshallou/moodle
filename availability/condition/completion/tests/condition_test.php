...
class availability_completion_condition_testcase extends advanced_testcase {
    ...
    public function test_in_tree() {
        ...
        $result = $tree->check_available(false, $course,
    ...
    public function test_usage() {
        ...
        $this->assertFalse($cond->is_available(false, $course,
                true, $user->id, $modinfo));
        $information = $cond->get_description(false, false, $course, $modinfo);
        $this->assertRegExp('~Page!.*is marked complete~', $information);
        $this->assertTrue($cond->is_available(true, $course,
                true, $user->id, $modinfo));

        // INCOMPLETE state (true).
        $cond = new condition((object)array(
                'cm' => (int)$pagecm->id, 'e' => COMPLETION_INCOMPLETE));
        $this->assertTrue($cond->is_available(false, $course,
                true, $user->id, $modinfo));
        $this->assertFalse($cond->is_available(true, $course,
                true, $user->id, $modinfo));
