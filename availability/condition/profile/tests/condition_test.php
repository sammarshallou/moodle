...
class availability_profile_condition_testcase extends advanced_testcase {
    ...
    public function test_is_available() {
        ...
        $cond = new condition($structure);
        $this->assertIsAvailableResult(false, '~Type of frog.*is not empty~',
                $cond, $SITE, $USER->id, $modinfo);
        $this->set_field($USER->id, 'poison dart');
        $this->assertIsAvailableResult(true, '~Type of frog.*is empty~',
                $cond, $SITE, $USER->id, $modinfo);

        $structure->op = condition::OP_IS_EMPTY;
        $cond = new condition($structure);
        $this->assertIsAvailableResult(false, '~.*Type of frog.*is empty~',
                $cond, $SITE, $USER->id, $modinfo);
        $this->set_field($USER->id, null);
        $this->assertIsAvailableResult(true, '~.*Type of frog.*is not empty~',
                $cond, $SITE, $USER->id, $modinfo);
        ...
        $structure->v = 'llf';
        $cond = new condition($structure);
        $this->assertIsAvailableResult(false, '~Type of frog.*contains.*llf~',
                $cond, $SITE, $USER->id, $modinfo);
        $this->set_field($USER->id, 'bullfrog');
        $this->assertIsAvailableResult(true, '~Type of frog.*does not contain.*llf~',
                $cond, $SITE, $USER->id, $modinfo);

        $structure->op = condition::OP_DOES_NOT_CONTAIN;
        $cond = new condition($structure);
        $this->assertIsAvailableResult(false, '~Type of frog.*does not contain.*llf~',
                $cond, $SITE, $USER->id, $modinfo);
        $this->set_field($USER->id, 'goliath');
        $this->assertIsAvailableResult(true, '~Type of frog.*contains.*llf~',
                $cond, $SITE, $USER->id, $modinfo);

        $structure->op = condition::OP_IS_EQUAL_TO;
        $structure->v = 'Kermit';
        $cond = new condition($structure);
        $this->assertIsAvailableResult(false, '~Type of frog.*is <.*Kermit~',
                $cond, $SITE, $USER->id, $modinfo);
        $this->set_field($USER->id, 'Kermit');
        $this->assertIsAvailableResult(true, '~Type of frog.*is not.*Kermit~',
                $cond, $SITE, $USER->id, $modinfo);

        $structure->op = condition::OP_STARTS_WITH;
        $structure->v = 'Kerm';
        $cond = new condition($structure);
        $this->assertIsAvailableResult(true, '~Type of frog.*does not start.*Kerm~',
                $cond, $SITE, $USER->id, $modinfo);
        $this->set_field($USER->id, 'Keroppi');
        $this->assertIsAvailableResult(false, '~Type of frog.*starts.*Kerm~',
                $cond, $SITE, $USER->id, $modinfo);

        $structure->op = condition::OP_ENDS_WITH;
        $structure->v = 'ppi';
        $cond = new condition($structure);
        $this->assertIsAvailableResult(true, '~Type of frog.*does not end.*ppi~',
                $cond, $SITE, $USER->id, $modinfo);
        $this->set_field($USER->id, 'Kermit');
        $this->assertIsAvailableResult(false, '~Type of frog.*ends.*ppi~',
                $cond, $SITE, $USER->id, $modinfo);
     ...
     */
    protected function assertIsAvailableResult($yes, $failpattern, condition $cond,
