...
class availability_date_condition_testcase extends advanced_testcase {
    ...
    public function test_in_tree() {
        ...
        $information = $tree->get_result_information($SITE, $modinfo, $result);
        $this->assertRegExp('~from.*18 February 2014, 7:20 PM~', $information);
    ...
    public function test_is_available() {
        ...
        condition::set_current_time_for_test($time - 1);
        $this->assertFalse($date->is_available(false, $SITE, true, $USER->id, $modinfo));
        condition::set_current_time_for_test($time);
        $this->assertTrue($date->is_available(false, $SITE, true, $USER->id, $modinfo));
    ...
    public function test_get_description() {
        ...
        // Test with >=.
        $date = new condition((object)array('d' => '>=', 't' => $time));
        $information = $date->get_description(true, false, $SITE, $modinfo);
        $this->assertRegExp('~after.*18 February 2014, 2:55 PM~', $information);
        $information = $date->get_description(true, true, $SITE, $modinfo);
        $this->assertRegExp('~before.*18 February 2014, 2:55 PM~', $information);
        $information = $date->get_standalone_description(true, false, $SITE, $modinfo);
        $this->assertRegExp('~from.*18 February 2014, 2:55 PM~', $information);
        $information = $date->get_standalone_description(true, true, $SITE, $modinfo);
        $this->assertRegExp('~until.*18 February 2014, 2:55 PM~', $information);
