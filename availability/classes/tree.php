...
class tree extends node {
    ...
    public function __construct($structure, $lax = false, $root = true) {
        ...
        foreach ($structure->c as $index => $child) {
            ...
            if (isset($child->type)) {
                ...
                $classname = '\availability_' .
                        clean_param($child->type, PARAM_PLUGIN) . '\condition';
    ...
    public function check_available($not,
        ...
        foreach ($this->children as $index => $child) {
            ...
            $childresult = $child->check_available(
                    $innernot, $course, $grabthelot, $userid, $modinfo);
     ...
     */
    public function get_full_information($course, \course_modinfo $modinfo) {
        if (!$this->root) {
            throw new coding_exception('Only supported on root item');
        }
        return $this->get_full_information_recursive(false, $course, $modinfo, null, true);
    }
     ...
     * @return string HTML data (empty string if none)
     */
    public function get_result_information($course, \course_modinfo $modinfo, result $result) {
        if (!$this->root) {
            throw new coding_exception('Only supported on root item');
        }
        return $this->get_full_information_recursive(false, $course, $modinfo, $result, true);
    }
    ...
    protected function get_full_information_recursive(
            $not, $course, \course_modinfo $modinfo, result $result = null, $root, $hidden = false) {
        global $PAGE;
        ...
        if (count ($children) === 1) {
            ...
            if ($child instanceof tree) {
                return $child->get_full_information_recursive(
                        $innernot, $course, $modinfo, $result, $root, $childhidden);
            } else {
                if ($root) {
                    $result = $child->get_standalone_description(
                            $staff, $innernot, $course, $modinfo);
                } else {
                    $result = $child->get_description(
                            $staff, $innernot, $course, $modinfo);
                }
                if ($childhidden) {
        ...
        foreach ($children as $child) {
            ...
            if ($child instanceof tree) {
                $items[] = $child->get_full_information_recursive(
                        $innernot, $course, $modinfo, $result, false, $childhidden);
            } else {
                $childdescription = $child->get_description(
                        $staff, $innernot, $course, $modinfo);
                if ($childhidden) {
