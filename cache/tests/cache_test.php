...
class cache_test extends \advanced_testcase {
    ...
    public function test_application_locking_multiple_layers_failures(): void {
        ...
        $definition = $ref->getProperty('definition')->getValue($cache);
        $localstore = $ref->getProperty('store')->getValue($cache);
        $sharedcache = $ref->getProperty('loader')->getValue($cache);
        $sharedstore = $ref->getProperty('store')->getValue($sharedcache);

        // Set the lock waiting time to 1 second so it doesn't take forever to run the test.
        $ref = new \ReflectionClass('\cachestore_file');
        $ref->getProperty('lockwait')->setValue($localstore, 1);
        $ref->getProperty('lockwait')->setValue($sharedstore, 1);
